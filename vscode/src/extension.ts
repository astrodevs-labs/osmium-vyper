...
async function launchFeatures() {
  const configuration = workspace.getConfiguration('Osmium-vyper');
  ...
  // I remove the condition for the slitherClient to be created
  // because I want to test the extension with the slither server
  // but the server is not running with the if
